package io.wireless.ball.android.ui.tinker;
...
import io.wireless.ball.android.cloud.ApiFacade;
import io.wireless.ball.android.cloud.api.Device;
import io.wireless.ball.android.cloud.api.TinkerResponse;
...
import io.wireless.ball.android.ui.ErrorsDelegate;
import io.wireless.ball.android.ui.corelist.CoreListActivity;
import io.wireless.ball.android.ui.tinker.Pin.OnAnalogWriteListener;
import io.wireless.ball.android.ui.util.NamingHelper;
...
public class TinkerFragment extends BaseFragment implements OnClickListener {
	...
	List<Pin> toggleActivationPins = Py.list();
	...
	private void loadViews() {
		...
		toggleActivationPins.addAll(allPins);
		...
		if (selectedPin.getConfiguredAction() != PinAction.NONE) {
			...
			switch (selectedPin.getConfiguredAction()) {
				...
				case TOGGLE_ACTIVATION:
					doToggleActivation(selectedPin);
					break;
				default:
					break;
			}
	...
	private void showTinkerSelect(Pin pin) {
		...
		final View toggleActivation = Ui.findView(selectDialogView, R.id.tinker_button_toggle_activation);
		final List<View> allButtons = list(analogRead, analogWrite, digitalRead, digitalWrite, toggleActivation);
		...
		toggleActivation.setOnTouchListener(new OnTouchListener() {

			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction() == MotionEvent.ACTION_DOWN) {
					setTinkerSelectButtonSelected(toggleActivation, allButtons);
				}
				return false;
			}
		});
		...
		toggleActivation.setOnClickListener(this);
		...
		if (!toggleActivationPins.contains(pin)) {
			toggleActivation.setVisibility(View.INVISIBLE);
		} else {
			toggleActivation.setVisibility(View.VISIBLE);
		}
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.tinker_button_digital_write:
				...
				break;
			case R.id.tinker_button_toggle_activation:
				onFunctionSelected(selectedPin, PinAction.TOGGLE_ACTIVATION);
				break;
